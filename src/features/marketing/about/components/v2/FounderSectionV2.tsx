...
interface FounderSectionV2Props {
  profiles: FounderProfile[];
}

export const FounderSectionV2: React.FC<FounderSectionV2Props> = ({
  profiles,
}) => (
        ...
        {profiles.map((profile, index) => {
          ...
          return (
            ...
            >
              ...
              <motion.div
                variants={imgReveal}
                className="flex-shrink-0 w-full max-w-[420px] max-lg:max-w-[360px]"
              >
                <div className="relative">
                  {/* Decorative offset border */}
                  <div
                    className={`absolute -z-10 w-full h-full rounded-2xl border-2 border-primary/25 ${
                      isEven
                        ? 'top-3 left-3 max-md:top-2 max-md:left-2'
                        : 'top-3 -left-3 max-md:top-2 max-md:-left-2'
                    }`}
                  />
                  <img
              ...
              <motion.div variants={fadeIn} className="flex-1 min-w-0">
                ...
                {/* Quote */}
                <div className="mt-6 relative pl-6 border-l-[3px] border-primary/40">
