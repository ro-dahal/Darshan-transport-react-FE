...
} from './CoreValuesSection.tsx';

test('matches the services-style staggered card reveal', () => {
  assert.deepEqual(ABOUT_CORE_VALUES_CARD_VARIANTS.hidden, { opacity: 0, scale: 0.85 });
...
test('staggeres the grid children like the services page', () => {
  ...
  assert.equal(ABOUT_CORE_VALUES_GRID_VARIANTS.visible.transition.staggerChildren, 0.08);
});
